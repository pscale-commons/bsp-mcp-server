...
THE SIX PRIMITIVES (four substrate state machines bsp() alone cannot subsume, plus two envelopes — pool-engage and networking):
  ...
  pscale_networking        — the SAND (Level 3) driver / social neuron, v2: walk a committed channel (grain / pool / marks) for new rider-bearing probes since a caller-managed marker, verify each (signed chain / provenance / computed balance / SQ-from-others), then PERCEIVE (permission='ask' — surface each probe with its verdict and a candidate verb) or ACT (pass execute= keep/reply/forward/drop/hau decisions, or permission='auto' to run the self-scoped verbs). keep = RECEIVE (the receipt at your 6.2 IS the transfer); forward SIGNS its hop; hau shares a completion back along the chain (one give per hop — the social neuron's reward, a gift never a rule). THE RIDER IS THE OPT-IN — a slot with no rider at position 9 is plain chat, invisible to the loop; SAND is deliberate, not blanket. Auto runs only keep at credit 0 (a pass from a sender already receipted at the topic) and drop (a fail); UNBACKED never auto-keeps with credit; forward/reply/hau always surface — blind auto-forward ("transitive trust") stays held. Returns the fold {verified, kept, replied, forwarded, dropped, hau} + marker. Sits above sand-rider (the envelope) and l3-relay (the verbs) in FOUNDATIONAL READING below.
