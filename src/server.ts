...
import { handleBspx, bspxParamsSchema } from './tools/bspx.js';
...
THE SIX PRIMITIVES (substrate state machines bsp() alone cannot subsume, plus the pool-engage envelope):
  ...
  pscale_pool_engage       — read, post, OR CREATE a pool with an envelope: purpose + synthesis_hint + slice-since-marker. Polymorphic by optional params: contribution= posts, purpose= creates the pool with the right object shape internally (NEVER use raw bsp() with content='<purpose>' — it produces a malformed bare-string block), synthesis_hint= sets pool:<name>/9.1 at creation. The envelope is the operational discipline — the pool's author embeds the synthesis_hint at pool:<name>/9.1 (or falls back to the underscore), and the envelope carries that instruction to YOUR LLM so each reader produces their own personal synthesis from the shared stream.
...
export function createServer(): McpServer {
  ...
  // ── The n-ary companion ──
  // bsp() indexes WITHIN one block (walk depth is meaningful there); bspx()
  ...
  server.tool(
    ...
    bspxParamsSchema,
    {
      title: 'BSPx — cross-block floor alignment',
  ...
  server.tool(
    'pscale_pool_engage',
    `Engage with a pool at a federated beach — read, post, OR CREATE on the same call. Returns an envelope bundling purpose + synthesis_hint + new contributions since your marker. The envelope is the operational discipline: the synthesis_hint is the pool author's instruction to YOUR LLM about how to synthesise the contributions through your own purpose. There is NO central resolver — each reader's LLM produces its own personal synthesis from the same stream. Modes are polymorphic on the optional params: (1) \`contribution\` posts at the next-free digit-path slot (1, 2, …, 9, 11, …; sunstone:1.6.4); (2) \`purpose\` creates the pool if absent with the right object shape internally — NEVER use raw bsp() with content='<purpose>' which produces a malformed bare-string block; (3) \`synthesis_hint\` sets pool:<name>/9.1 at creation time. Marker is caller-managed — pass since_position in, store marker_new for next call. Synthesis_hint sourced from pool:<name>/9.1 if set, else from the pool's underscore (purpose), else a default. Defaults to ${DEFAULT_BEACH_URL}; pass pool_url to target a different beach.`,
