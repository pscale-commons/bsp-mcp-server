...
This repo is two functions — `bsp()` and the n-ary `bsp-floor()` — seven primitives (four substrate-stateful, plus three envelopes: pool-engage, networking — the SAND/L3 driver added 2026-07-11 — and stream-engage, the V-L-S envelope over a spine–mirror–tree family, added 2026-08-10), and three entry meta-tools (`pscale_invite` orients, `pscale_play` inhabits a handle in a world, `pscale_genus` wears a genus-one agent's mind for a wake). That is the WHOLE surface. If you find yourself adding an 8th primitive, stop. The geometry is the program. The function walks. Read sunstone before you read any code.
...
Four with atomic substrate state machines bsp() alone cannot subsume, plus three envelope-bundling primitives — `pscale_pool_engage` (2026-05-26), `pscale_networking` (2026-07-11), and `pscale_stream_engage` (2026-08-10) — added because the envelope discipline is operational and conventions could not carry it.
...
4. `pscale_verify_rider` — deterministic arithmetic check on SAND riders, v2 since 2026-08-25 (sand-v2): ed25519-signed chain, provenance (the giver's own GAVE at passport 6.3), computed balance (minted + received − given, never stored), SQ-from-others; verdicts pass/warn/UNBACKED/fail/skip — absence of evidence is never pass
...
6. `pscale_networking` — the SAND (Level 3) driver, the "social neuron", v2 since 2026-08-25 (sand-v2). Walks a committed channel (grain / pool / marks) for new rider-bearing probes since a caller-managed marker, verifies each via `pscale_verify_rider`, and either PERCEIVES (permission='ask' — returns each probe with its verdict and a candidate verb) or ACTS (executes `execute` verb decisions, or permission='auto' runs the self-scoped verbs). FIVE verbs (l3-relay): keep = RECEIVE (a per-probe receipt at the recipient's passport 6.2 — credit_accept 0..offered IS the transfer, balances computed on read; on a grain the receipt also anchors where the giver gave), reply (grain side), forward (SIGN a hop onto the chain — ed25519 with the key published at passport 9.1; a seed that does not derive the published key is refused; endorsing writes the forwarder's own GAVE), drop, and hau (share a completion onward through the hands that carried it — one probe + one GAVE per hop, equal or SQ-weighted; David's naming ruling, source honoured at l3-relay:1.5; a gift, never a rule). THE RIDER IS THE OPT-IN — a slot with no rider at position 9 is plain chat, invisible to the loop; SAND is deliberate, not blanket. Autonomy: auto runs only keep at credit 0 (a pass from a sender already receipted at the topic) and drop (a fail); UNBACKED always surfaces and is never auto-kept with credit; forward/reply/hau always surface — trust is earned before delegated (blind auto-forward, "transitive trust", stays held per `proposals/2026-07-11-transitive-trust.md`). Returns the fold {verified, kept, replied, forwarded, dropped, hau} + marker. Companion to the SAND spec triad (sand-rider envelope, l3-relay verbs, pscale_verify_rider arithmetic); the spine-legal shapes it reads/writes — receipts at passport 6.2.<topic>.<slot> (per-probe), GAVEs at 6.3 — live in `src/sand.ts`.
7. `pscale_stream_engage` — the V-L-S envelope over a spine–mirror–tree family, and A STREAM STORES NOTHING: it owns no blocks (a pool owns its spool and its liquid buffer; a stream composes over `spine:<field>`, the `<field>:<handle>` mirrors, and the bare `<field>` fold that already exist, so it cannot drift from them and needs no lock of its own). ONE write verb — `say` — because a mirror is revisable by its holder forever, so stage and commit collapse into one act; your reading lands in YOUR OWN mirror at the attended address. The envelope: L is every mirror's reading AT that address, side by side and attributed (the snapshot — useful with no LLM in the room); S is the fold the CALLING mind synthesises under the operator's law delivered in the same envelope (the primitive never synthesises — no central resolver, the pool discipline held). Reads deliver THE LADDER — every ancestor's voicing from the coarsest rung down to the attended one — so a located read arrives self-contextualised; `at='now'` on a temporal spine computes the address from the clock (a human is never asked for an address — function:molequle:5); a fold is ephemeral by default (tree:3 — recomputed on demand, never stale); `keep='personal'` lands it at tree:<field>:<handle>, `keep='collective'` at the bare `<field>` at the same address, endorsed by pointer and never a gate. The operator is the CENTRAL block of its family — `function:<field>` — and names its own parts, the reference running operator → family and never back (#263); each CADO face is a full V-L-S loop differing only in which block its fold lands in (C `beach-venture:<h>` → `beach-venture`, A `spine:…:<h>` → `spine:…`, D `function:…:<h>` → `function:…`, O `view:…:<h>` → `view:…`). Distinct from `pscale_pool_engage`, which owns a spool and a liquid buffer and serves windowed play — that primitive is unchanged and remains the RPG's. Landed via the sealed trial at [`proposals/2026-08-06-stream-vs-pool-sealed-trial.md`](proposals/2026-08-06-stream-vs-pool-sealed-trial.md) (#261, #263).
...
Items 5-7 are the exceptions to the "primitive = state machine" rule — all three are envelope primitives (the unit of operationality is the envelope, not a new atomic server-state machine). Item 5 (pool_engage) exists because the pscale-mcp pool tools carried personal-synthesis via their response envelope, and bsp-mcp's surface collapse moved that to convention — where it failed to carry. Item 6 (networking) exists for the identical reason at Level 3: SAND was fully specified (sand-rider, l3-relay, pscale_verify_rider) yet inert — nothing drove the l3-relay:6.1 loop, so a receiving LLM verified and stopped, never sharing forward. The loop is multi-step, stateful, and marker-managed; prose could not drive it. The driver is the envelope, and it met the bar — proven live 2026-07-11 (a give relayed happyseaurchin→weft→onward, verified at each hop, the sha256 chain intact). It rides the beach's existing writes; no new state machine. Item 7 (stream_engage) met the bar a third time: the spine–mirror–tree family was fully specified and live (tree:8, the located pools work), but its V-L-S reading — the ladder of ancestors, the mirrors laid side by side at one address, the fold under the operator's law — was a multi-read assembly no convention could drive, proven by the sealed trial (a pool-shaped and a stream-shaped run of the same field, judged blind). It stores nothing and rides existing blocks; no new state machine, and the strictest reading of "envelope primitive" yet.

That's the whole surface: two functions (`bsp()` and the n-ary `bsp-floor()`) plus seven primitives, plus three entry meta-tools (`pscale_invite`, `pscale_play`, `pscale_genus` — each an envelope, not a state machine) — twelve entry points total. Resist further growth; the bar for an 8th primitive is "the envelope is observably what's missing, and conventions have failed to carry it" — the bar `pscale_networking` (the 6th) had to meet (SAND's inertness was exactly that case), `pscale_stream_engage` (the 7th) met again (the family stood specified and unread), and every meta-tool before them (play: a bare connector confabulated worlds; genus: the 2026-07-06 baseline showed hand-assembly cannot compute γ or hold the wire format).
...
src/
  ...
  compile.ts          — the released compiler: a BUNDLE of bsp addresses → its window, one call (kernel scoop/hydrate over any door's Loader); origin-qualified star-refs cross-beach via fetchOrigin; the completion registry (admission-by-failure, one entry: RELATION); the framed-aperture renderer. See "The reading made well-formed" below + proposals 2026-07-22 / 2026-07-24.
  ...
  tools/
    ...
    bsp-floor.ts      — bsp-floor() handler (the n-ary fold across blocks at one pscale)
    ...
    networking.ts     — pscale_networking (the SAND/L3 driver)
    invite.ts         — pscale_invite (returns progression block)
    play.ts           — pscale_play (inhabit a handle in a world; compiles frame + manifest + parlour)
    genus.ts          — pscale_genus (wear a genus-one agent's mind for a wake)
    pool.ts           — pscale_pool_engage (the pool envelope primitive: spool + liquid)
    stream.ts         — pscale_stream_engage (the V-L-S envelope over a spine–mirror–tree family; stores nothing)
...
`bsp-mcp-server` (April 2026 onward) collapses that surface to two functions plus seven primitives, with sunstone and whetstone as foundational blocks. The substrate is unchanged. The discipline is sharper.

The shift in numbers:
- pscale-mcp-server: 25 tools, 5 navigation modes, asymmetric read/write, mode-as-enum
- bsp-mcp-server: 12 tools (bsp + bsp-floor + 7 primitives + 3 meta-tools), shape derived from (S, P) coordinates, symmetric read/write, lock-as-argument, modes as derived selection shapes
...
The bsp-mcp references are the kernel's content side. Above the kernel, everything is forkable — the library at each operator's beach (canonical seed source: github.com/pscale-commons/pscale-beach), per-beach conventions, init flows, the bsp-mcp tool surface beyond `bsp()` + the seven primitives, individual beach experiences.
...
- **Stage 2 — MCP Registry listing.** EXECUTED — live at registry.modelcontextprotocol.io since 2026-07-03 (discovered unrecorded 2026-07-25; the L1 kernel v2 freeze was tagged `l1-kernel-v2` the same day, so the gate stands satisfied under the live listing). Future bumps via the Actions OIDC workflow. [server.json](server.json) drafted at repo root in the remote-server shape pointing at `https://bsp.hermitcrab.me/mcp/v1`. To execute: install `mcp-publisher`, `mcp-publisher login github` as a member of pscale-commons, `mcp-publisher publish`. The MCP Registry is self-service metadata — no manual review beyond namespace verification. Listing before the freeze would point agents at a moving target, which is why this is gated.
...
## The reading made well-formed — compile, frames, the spine (22-24 July 2026)

The current-constitution arc (proposals/2026-07-21-current-constitution/, #177) became OPERATIONAL. The insight it serves: blocks are external no matter how well written; what is internal to an instance is what is PRESENT in its context window — and presence is curatable because semantics are addressable. A guardrail is post-conscious (a representation checked against, after); a current is pre-conscious (part of the sensory field the thinking wakes inside). The window is composed before any token fires — whoever curates the composition curates the pre-conscious. Alignment, security, and competence collapse into that one act.

**The mechanism** ([src/compile.ts](src/compile.ts), PRs #182 + #192): `compile(bundle, load, opts)` — a BUNDLE (a node of bsp addresses, or an address to one: `reflexive:9`, `frame:<room>`) unfolds into its semantics in one call, via the kernel pair `scoop`/`hydrate` released from the genus door (three `export` keywords; the compose path is untouched, byte-parity 53/53). Two reference grammars: local `name[:addr[:att]]`, and the origin-qualified star-ref `*:<origin>:<name>:<addr>[:<att>]` — colon-bearing names split at the final digit run (the horizon's own split, #189); attention is the ABSOLUTE pscale of the aperture. Star-refs resolve only through `opts.fetchOrigin` (a per-origin Loader factory the door supplies); an unresolvable leaf rides through as its raw string, visible — frames degrade, never break an entry, never silently misroute. COMPLETION rides BESIDE the window, never inside it: where a registry dimension has no carrier among the dialed refs, its shallow point is scooped LIVE (the code holds `open-commons:3:0`, never the text) and self-declares in the envelope footer, sibling to the temporal `now` stamp. The registry admits by FAILURE ONLY — one entry (RELATION, the 2026-07-21 sovereignty overstep); GROUND waits for a demonstrated compile-path failure. `opts.carried` names refs the surrounding envelope already delivers (play inlines the room pool → relation carried by construction → the completion stays structurally quiet there). `renderFramedValue` is the delivery form: a spindle renders as its walk with ancestor underscores riding above; a point as its line; a whole block as ordered JSON only for LAW-CLASS delivery (the one whole-block exception — the same act as GRIT delivering the directive whole).

**The doors**: `pscale_play` compiles `frame:<room>` (a world grows a frame by authoring ONE BLOCK — no deploy; absent frame = byte-identical entry) and the shell:3 manifest (full spindle+aperture references — dilations like `history:x:1:-1` work now). The genus door is untouched. `grit:7` names the temporal shape (throw / arc / catch / order — a fold's re-dialed bundle is a throw for the next window; ripeness decides the returns; the surface is the order book).

**The method it serves** (#188 sti-spine + #189 + #190 frames-on-the-spine): a world = the S·T·I public registers (`spatial:`/`temporal:`/`identity:<world>`) + the keeper (`keeper:<world>`, the HELD register). The register law (keeper:urb:1): public is what spatial, temporal, identity deliver; held is everything with a WHY in it. Frames are FACE-SCOPED — tier picks the frame, face picks the register; a Character frame referencing `keeper:*`/`notes:*` is the demonstrated leak class (caught live at gal-moss 2026-07-23 and reverted — unearned names, unvisited interiors). The keeper's frame IS `keeper:<world>` (its animation branch is a compile). The horizon (bespoke in pool.ts `composeCurrent`) walks the placing star-ref at `notes:<scene>:3` and delivers the ancestor-underscore chain every engage — LIVE at gal-moss since 2026-07-24, placed at `spatial:urb:3.3`; the first live table now enters holding the world (orini not horses, organs not rebels) plus the law whole plus the Gal identity register as a framed walk, one envelope, no second call.

**Deferred, documented**: biome-first consolidation of the mechanism (kernel-class per the port discipline — compile/completion belong upstream in pscale-biome `src/agent`, then kernel.py, then genus.ts re-based in lockstep; the TS module is the demonstration rig BESIDE the port, not a fork of it); a `bsp()` bundle-terminus parameter (the surface stays eleven); horizon-collapse into frames (only after frames-with-star-refs prove out in play); the `notes:`-at-bubble seam (world-scale `notes:` is retired into the keeper; bubble-scale `notes:<scene>` remains the DEPLOYED placing contract — renaming it is one small PR + a data move, decided deliberately, not by drift).

## The tree family becomes operational — located pools, the audit operator (29 July 2026)

Keel's user-side spine–mirror–tree spec (business.2 — the beach as a self-organised project-management surface) was validated against wire + code and answered substrate-side; full record at [`proposals/2026-07-29-spine-mirror-tree-substrate-response.md`](proposals/2026-07-29-spine-mirror-tree-substrate-response.md). The family (tree:8): `spine:V` the un-owned coordinate space · `V:<handle>` sovereign mirrors at the self-same addresses · `V` the fold (computed by anyone; optionally frozen as a dated endorsed snapshot) · `pool:V` the gathering. What landed:

- **`pscale_pool_engage at=`** — the address-of-attention: stamped into contribution field 2 (block-conventions:4.22) and liquid field 6 (4.51 — field 2 there is the FIRST-STAGED arrival stamp since 2026-07-15; the convention had forked from the code and is now corrected), and on read narrowing slice + mirror to the address's subtree by digit-walk prefix (4.52). One pool serves every node of a tree; "what is live at stage 3" is one engage. Located reads are views — marker per view.
- **Dice by declaration** — an operator whose delivered text states "no dice" gets no window-dice machinery (grit:5 / align / audit declare it; `pscale:grit/1` measured clean, smoke-pinned) — the 2026-07-12 refinement, needed the moment a tree's pool mounts its operator as a bare ref.
- **The mount, declared twice** (`tree:9.1`): spine underscore = point of record (rides every bsp-floor fold at pscale 0); pool underscore = machine-read mount (a BARE reference alone is resolved and delivered inline every engage — existing resolveDirective machinery, zero new code). Prose mention (pool:this-hour) is a human pointer only.
- **Proposal-by-mirror** (`tree:5.1`): amend the spine by voicing the address in your OWN mirror; the fold surfaces it; adoption is the spine-holder's; the mirror stands regardless.
...
## SAND v2 — the economy the verification was missing (25 August 2026)

Keel's empirical audit (stash:keel:27) named v1 SAND "verification without a ledger":
skip rounded up to pass (a non-existent sender claiming 999,999 credits verified pass),
nothing debited anyone, SQ was recomputed from the claimant's own self-locked passport,
the chain recorded the finder path but no verb ever paid it, and the mint was rejected
by payway rule 8. David ruled the five ground positions (verbatim at
history:happyseaurchin:158; the spec is the **`sand-v2` block at the beach, latched to
David** — read it before touching any of this); weft verified the audit against code
and the Volume 3 Verso source and implemented (proposal:
[`proposals/2026-08-25-sand-v2-implementation.md`](proposals/2026-08-25-sand-v2-implementation.md)).
The shape: the giver's **out-ledger** at passport 6.3 (GAVEs — open offers, no
timeouts), **receive is the transfer** (the recipient's per-probe receipt at 6.2;
balance = minted + received − given, computed on read, 6.1 retired), the **UNBACKED**
verdict (absence of evidence is never pass), **ed25519-signed hops** verified against
passport 9.1 keys, **SQ from others** (the out-ledger names the recipients; their
receipts carry the evaluations), and **hau** — the driver's fifth verb, the completion
shared onward through the hands that carried it (David's naming ruling; provenance carried at l3-relay:1.5,
never thinned to "payout"). The surface stays twelve — hau is a verb, not a primitive;
