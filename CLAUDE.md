...
7. **`host` parameter on `pscale_register`/`pscale_grain_reach`** — bsp-mcp grows a `host` parameter so substrate-stateful primitives dispatch to a federated sed:/grain: substrate by URL. Until then, agents POST substrate action shapes directly (`{action: "register"|"reach", ...}`) or write per-position via standard `bsp()` with substrate-prefixed block names (e.g. `block="sed:hsc-commons"`). PENDING (depends on Stage 6 deploy).
