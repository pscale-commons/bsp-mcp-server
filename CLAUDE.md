...
**The play, as a player meets it — the settled loop every change is checked against.** A player types what their character intends, and the text they read back confirms what is already in their head and fleshes it out with continuity as it unfolds — attention about 30% history, 70% what is happening and what is intended. Each moment is pregnant with possibilities: the others' vapour (typing, presence) and liquid (their staged intentions) stand beside one's own. Two verbs, no more — **say** (vapour → liquid) and **make it happen** (liquid → solid): the committing LLM resolves the moment under the room's law, the mechanics (a move included) execute on that resolution with no confirmation — a confirmation kills the experience — and each player's own LLM derives their narrative. The soft-LLM is the player's own, in vapour: "what do I see?" is answered privately from where the character stands and commits nothing; the LLM reads the words, never a keyword. A character's thread is that character's whole story, from their account, across every room. The mirror stays clean, a magic mirror: place, ways, record and addresses are a drawer or an o-page, for author, designer and observer, and the player is never sent notes. A mechanical change that makes a player confirm, read notices, or describe this again is the wrong change.
